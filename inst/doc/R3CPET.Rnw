...
\email{nde12@mails.tsunghua.edu.cn}
}
...
The \Rmethod{clusterInteractions} can be used to do so. 

The \Rmethod{clusterInteractions} is defined as follow:

<<cluster_usage, tidy=TRUE,  eval=FALSE>>=
clusterInteractions(object, method=c("sota"), nbClus=6 )
@

By  default number of clusters is 20.
<<clusterInteractions, tidy=TRUE,cache=TRUE, prompt=TRUE>>= 
## clustering 
hlda <- clusterInteractions(hlda, nbClus=6)
@
